...
async function runTests() {
  try {
    ...
    assert(strippedConfig.commitTypes.length === defaultConfig.commitTypes.length, 'Commit types should be preserved');
    ...
    assert(diff.some(d => d.path === 'emojiEnabled'), 'Should detect emojiEnabled change');
    assert(diff.some(d => d.path === 'maxSubjectLength'), 'Should detect maxSubjectLength change');
    ...
    try {
      ...
      assert(output.includes('***REDACTED***') || !output.includes('"apiKey"'), 'API keys should be redacted or excluded');
    } finally {
      restoreConsole();
      capturedLogs = [];
    }
    ...
    try {
      ...
      process.exit = (() => { importFailed = true; }) as any;
    ...
    assert(importedConfig.maxSubjectLength === 60, 'Imported config should have maxSubjectLength: 60');
    ...
    try {
      ...
      const output = capturedLogs.join('\n');
      assert(output.includes('Configuration Health Check'), 'Doctor should show health check header');
    ...
    assert(resetConfig_result.emojiEnabled === defaultConfig.emojiEnabled, 'Reset should restore emojiEnabled default');
    assert(resetConfig_result.maxSubjectLength === defaultConfig.maxSubjectLength, 'Reset should restore maxSubjectLength default');

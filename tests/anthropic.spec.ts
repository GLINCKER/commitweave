...
import { AnthropicProvider } from '../src/utils/ai.js';
...
async function assertRejects(promise: Promise<any>, errorType?: any, message?: string) {
  try {
    ...
    if (errorType && !(error instanceof errorType)) {
      throw new Error(`❌ Expected error of type ${errorType.name}, got ${error.constructor.name}${message ? `: ${message}` : ''}`);
    }
...
async function runTests() {
  try {
    ...
    assert(!unconfiguredProvider.isConfigured(), 'Provider should not be configured without API key');
    ...
    assert(rateLimitError.name === 'ClaudeRateLimitError', 'ClaudeRateLimitError should have correct name');
    assert(validationError.name === 'ClaudeValidationError', 'ClaudeValidationError should have correct name');
    ...
    assert(defaultConfig.claude !== undefined, 'Default config should include Claude configuration');
    assert(defaultConfig.claude?.enabled === false, 'Claude should be disabled by default');
    assert(defaultConfig.claude?.model === 'claude-3-haiku-20240307', 'Should default to haiku model');

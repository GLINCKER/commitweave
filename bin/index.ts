...
async function main() {
  ...
  if (interactive && useFancyUI) {
    const { printBanner, showLoadingAnimation, printFeatureHighlight } = await lazy(
    ...
    if (!isConfig && !flags.version) {
      console.log(
        chalk.hex(BRAND_COLORS.muted)('Powered by ') +
  ...
  try {
    ...
    if (flags.export) {
      const { exportConfig, parseExportArgs } = await lazy(
        () => import('../src/cli/commands/exportConfig.js')
      );
    ...
    if (flags.import) {
      const { importConfig, parseImportArgs } = await lazy(
        () => import('../src/cli/commands/importConfig.js')
      );
    ...
    if (flags.reset) {
      const { resetConfig, parseResetArgs } = await lazy(
        () => import('../src/cli/commands/resetConfig.js')
      );
...
async function handleCreateCommand(useFancyUI: boolean = false) {
  try {
    ...
    if (useFancyUI) {
      ...
      console.log(
        chalk.hex(BRAND_COLORS.primary)('╭─────────────────────────────────────────────╮')
      );
      console.log(
        chalk.hex(BRAND_COLORS.primary)('│') +
          chalk.bold.white('         🚀 Commit Creation Wizard           ') +
          chalk.hex(BRAND_COLORS.primary)('│')
      );
      console.log(
        chalk.hex(BRAND_COLORS.primary)('╰─────────────────────────────────────────────╯')
    ...
    if (useFancyUI) {
      const { showLoadingAnimation } = await lazy(() => import('../src/ui/banner.js'));
      console.log(
        chalk.hex(BRAND_COLORS.accent)('\n╭─────────────────────────────────────────────╮')
      );
      console.log(
        chalk.hex(BRAND_COLORS.accent)('│') +
          chalk.bold.white('         📦 Finalizing Your Commit          ') +
          chalk.hex(BRAND_COLORS.accent)('│')
      );
      console.log(
        chalk.hex(BRAND_COLORS.accent)('╰─────────────────────────────────────────────╯')
    ...
    console.log(
      chalk.hex(BRAND_COLORS.success).bold('\n🎉 Success! ') +
        chalk.hex(BRAND_COLORS.success)(commitResult)
    );
...
async function handleAICommitCommand(useFancyUI: boolean = false) {
  try {
    ...
    if (!aiConfig || (!aiConfig.apiKey && aiConfig.provider !== 'mock')) {
      ...
      console.log(
        chalk.gray('   This will generate placeholder commit messages for demonstration')
      );
      console.log(
        chalk.cyan('   💡 To enable real AI: configure your API key in ') +
          chalk.white('glinr-commit.json')
      );
      console.log('');
    ...
    try {
      ...
      console.log(
        chalk.gray('   Or navigate to an existing repo: ') + chalk.cyan('cd your-project')
      );
    ...
    console.log(
      chalk.gray(`   📊 ${diffStats.additions} additions, ${diffStats.deletions} deletions`)
    );
      ...
      console.log(
        chalk.gray(
          `   📁 Files: ${diffStats.files
            .slice(0, 3)
            .map(f => chalk.cyan(f))
            .join(', ')} and ${diffStats.files.length - 3} more...`
        )
      );
    }
    console.log('');
    ...
    const { subject, body } = await withProgress(
  ...
  try {
    ...
    if (validation.valid) {
      ...
      if (parsed.type) {
        ...
        if (commitType) {
          console.log(
            chalk.gray(`   Type: ${commitType.emoji} ${parsed.type} - ${commitType.description}`)
          );
        }
      }
    } else {
...
async function handleConfigSubmenu() {
  ...
  switch ((response as { configAction: string }).configAction) {
    ...
    case 'import':
      console.log(
        chalk.yellow('💡 To import a configuration, use: commitweave import <path-or-url>')
      );
      break;
    case 'doctor':
      const { doctorConfig } = await lazy(() => import('../src/cli/commands/doctorConfig.js'));

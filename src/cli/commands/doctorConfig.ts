...
export async function doctorConfig(): Promise<void> {
  await handleAsync(async () => {
    ...
    if (failCount === 0 && warnCount === 0) {
      ...
      console.log(chalk.red(`💥 Configuration has ${failCount} error(s) that should be addressed.`));
    }
    
    console.log(chalk.gray(`   Summary: ${passCount} passed, ${warnCount} warnings, ${failCount} errors`));
    console.log('');
    
    if (failCount > 0 || warnCount > 0) {
      console.log(chalk.gray('💡 Use ') + chalk.cyan('commitweave reset') + chalk.gray(' to restore defaults'));
      console.log(chalk.gray('💡 Use ') + chalk.cyan('commitweave list') + chalk.gray(' to view current settings'));
    }

...
export async function importConfig(source: string, options: ImportOptions = {}): Promise<void> {
  await handleAsync(async () => {
    ...
    try {
      ...
      throw new Error(
        `Failed to load configuration from ${source}: ${error instanceof Error ? error.message : 'Unknown error'}`
      );
    }
    ...
    if (!versionCheck.valid) {
      ...
      console.log(
        chalk.yellow('💡 Please update the configuration to version "1.0" before importing.')
      );
      process.exit(1);
    }
